...
class ManageSectionController extends Controller
{
    ...
    public function sectionElementCreate(Request $request)
    {
        ...
        if (array_key_exists('slug_title', $section)) {
        ...
        if (in_array('file', array_values($section))) {
            //$key = array_search('file', $section);
            $key_dump = array_keys($section, 'file');

            $size = $section['size'] ? $section['size'] : Null;


            foreach ($key_dump as $key_data) {

                if ($key_data && $request->hasFile($key_data)) {
                    $response = cloudUpload($size, $request->$key_data, $request->name, '');
                    $filename = $response;
                    $data[$key_data] = $filename;
                }
            }
        }
        SectionData::create([
