...
class SiteController extends Controller
{
    public function index()
    {
        ...
        $seo_description = "{$sections->seo_description}";
        if (!$sections) {
    ...
    public function page(Request $request)
    {
        ...
        return view('frontend.pages', compact('pageTitle', 'page', 'seo_description'));
    }
    ...
    {
        $product_details = SectionData::where("key", "product.element")->where('data->slug', $slug)->first();
        $related_product = SectionData::where('data->slug', "!=", $slug)
            ->where("key", "product.element")
            ->inRandomOrder()
            ->limit(5)
            ->get();

        return view('frontend.sections.product_details', compact('product_details', 'related_product'));
    }
    public function press_release($slug)
    {
        $press_release = SectionData::where('data->slug', $slug)
            ->first();
        return view('frontend.sections.press_release_details', compact('press_release'));
    }
    public function news_and_events($slug)
    {
        $news_and_events = SectionData::where('data->slug', $slug)
            ->first();

        return view('frontend.sections.news_and_events_details', compact('news_and_events'));
    }
    
    public function sister_concern($slug)
    {
        $sister_concern = SectionData::where('data->slug', $slug)
            ->first();

        return view('frontend.sections.sister_concern_details', compact('sister_concern'));
    }
    public function career_details($slug)
    {
        $career_details = SectionData::where('data->slug', $slug)
            ->first();
        return view('frontend.sections.job_details', compact('career_details'));
    }
